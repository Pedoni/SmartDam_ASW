...
function listen (port, debug) {
  var javapath = process.platform != 'win32' ? 'java'
    : fs.existsSync('c:\\windows\\syswow64') ? 'c:\\windows\\syswow64\\java.exe'
    : 'c:\\windows\\system32\\java.exe';

  var java = spawn(javapath, ['-cp', './jSSC-2.6.0-Release/jssc.jar' + path.delimiter + '.', 'Main', port], {
    cwd: __dirname
  });
  java.on('error', function (err) {
    if (err && err.code == 'ENOENT') {
      // Assuming it's not installed
      throw new Error("Java is not installed or could not be found on your system (attempted to execute " + javapath + "). Please install java to enable serial communication with jssc.");
    } else {
      throw err;
    }
  })
